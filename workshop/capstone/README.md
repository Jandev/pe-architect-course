...
### Teardown — Reverting IDP RBAC Restrictions

After demoing the platform, run the following commands to remove the RBAC and Gatekeeper resources added as part of issue #13. This leaves the cluster in the state it was in before the Argo Rollouts integration.

```bash
# ─── Gatekeeper — RequireIDPOrigin ───────────────────────────────────────────
# Remove the constraint first (it depends on the template), then the template.
kubectl delete requireidporigin require-idp-origin
kubectl delete constrainttemplate requireidporigin

# ─── Gatekeeper — RequireArgoRollouts (revert to production-only scope) ─────
# Reapply the constraint and template from the previous version, or delete the
# constraint and redeploy from git main if you no longer need it at all.
kubectl delete requireargorollouts require-argo-rollouts
kubectl delete constrainttemplate requireargorollouts

# ─── IDP RBAC — production namespace ─────────────────────────────────────────
kubectl delete rolebinding idp-creator-binding -n production
kubectl delete role idp-creator -n production

# ─── IDP RBAC — cluster-scoped ───────────────────────────────────────────────
kubectl delete clusterrolebinding idp-namespace-creator-binding
kubectl delete clusterrole idp-namespace-creator

# ─── IDP RBAC — team namespaces (generated by the operator) ──────────────────
# Repeat for each team-* namespace that exists in your cluster.
for ns in $(kubectl get namespaces -l app.kubernetes.io/managed-by=teams-operator -o name | sed 's|namespace/||'); do
...
kubectl get clusterrole idp-namespace-creator 2>/dev/null || echo "ClusterRole removed ✓"
kubectl get clusterrolebinding idp-namespace-creator-binding 2>/dev/null || echo "ClusterRoleBinding removed ✓"
```
