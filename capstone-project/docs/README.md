...
| **Teams CLI (`tli`)**  | `.NET 10` file-based app (`System.CommandLine`); calls the Teams API directly over HTTP |
...
- Holds state in-memory; **restart-resilient** — on startup it queries the Kubernetes API for
  ...
  store from their labels and annotations
- A `ServiceAccount` with a `ClusterRole` (get/list namespaces) enables this Kubernetes API access
...
3. Manual promotion  (autoPromotionEnabled: false)
   └─ kubectl argo rollouts promote rollout-demo-api
      ├─ Active service selector switches to green pods
      └─ Blue ReplicaSet is scaled down
