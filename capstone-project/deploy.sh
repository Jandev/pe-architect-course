...
# ─── 1. Helm Repositories ────────────────────────────────────────────────────
echo "[1/25] Adding Helm repositories..."
...
# ─── 2. Ingress-Nginx ────────────────────────────────────────────
echo "[2/25] Installing Ingress-Nginx..."
...
# ─── 2. Grafana Stack ────────────────────────────────────────────────────────
echo "[3/25] Installing Grafana stack (kube-prometheus-stack)..."
...
# ─── 3. Gatekeeper ───────────────────────────────────────────────────────────
echo "[4/25] Installing OPA Gatekeeper..."
...
echo "[5/25] Applying Gatekeeper namespace-labels constraint..."
...
echo "[6/25] Applying CVE vulnerability scanning constraint..."
...
echo "[7/25] Applying code quality (coverage) constraint..."
...
echo "[8/25] Installing Falco runtime security..."
...
echo "[9/25] Installing Kubescape compliance scanning..."
...
echo "[10/25] Applying Gatekeeper secops (root prevention) constraint..."
...
# ─── 10. Metrics Server ──────────────────────────────────────────────────────
echo "[11/25] Installing Metrics Server..."
...
# ─── 11. Deploy Keycloak ─────────────────────────────────────────────────────
echo "[12/25] Deploying Keycloak to Kubernetes..."
...
echo "[15/25] Building Teams API Docker image..."
...
if [ "$IS_ORBSTACK" = "true" ]; then
  echo "[16/25] Skipping kind image load on OrbStack (local Docker images are directly accessible)."
else
  if [ -z "$KIND_CLUSTER" ]; then
    echo "[16/25] WARNING: No kind cluster found — skipping image load. Run 'kind load docker-image teams-api:local --name <cluster>' manually."
  else
    echo "[16/25] Loading Teams API image into kind cluster '${KIND_CLUSTER}'..."
...
echo "[17/25] Deploying Teams API to Kubernetes..."
...
echo "[18/25] Waiting for Teams API rollout..."
...
echo "[19/25] Building Teams UI Docker image..."
...
if [ "$IS_ORBSTACK" = "true" ]; then
  echo "[20/25] Skipping kind image load on OrbStack (local Docker images are directly accessible)."
else
  if [ -z "$KIND_CLUSTER" ]; then
    echo "[20/25] WARNING: No kind cluster found — skipping image load. Run 'kind load docker-image teams-ui:local --name <cluster>' manually."
  else
    echo "[20/25] Loading Teams UI image into kind cluster '${KIND_CLUSTER}'..."
...
echo "[21/25] Deploying Teams UI to Kubernetes..."
...
echo "[22/25] Building Teams Operator Docker image..."
...
if [ "$IS_ORBSTACK" = "true" ]; then
  echo "[23/25] Skipping kind image load on OrbStack (local Docker images are directly accessible)."
else
  if [ -z "$KIND_CLUSTER" ]; then
    echo "[23/25] WARNING: No kind cluster found — skipping image load. Run 'kind load docker-image teams-operator:local --name <cluster>' manually."
  else
    echo "[23/25] Loading Teams Operator image into kind cluster '${KIND_CLUSTER}'..."
...
echo "[24/25] Deploying Teams Operator to Kubernetes..."
