...
export class TeamsService {
  ...
  createTeam(team: TeamCreate): Observable<Team> {
    ...
    return this.http.post<Team>(url, team).pipe(catchError(this.handleError));
  }
  ...
  deployApp(teamId: string, req: DeployRequest): Observable<any> {
    const url = `${this.apiUrl}/teams/${teamId}/deploy`;
    return this.http.post(url, req).pipe(catchError(this.handleError));
  }

  promoteApp(teamId: string, req: PromoteRequest): Observable<any> {
    const url = `${this.apiUrl}/teams/${teamId}/promote`;
    return this.http.post(url, req).pipe(catchError(this.handleError));
  }

  rollbackApp(teamId: string, req: RollbackRequest): Observable<any> {
    const url = `${this.apiUrl}/teams/${teamId}/rollback`;
    return this.http.post(url, req).pipe(catchError(this.handleError));
  }
  ...
  private handleError = (error: HttpErrorResponse) => {
    ...
    if (error.status === 401) {
      ...
      // Server-side error
      errorMessage =
        error.error?.detail || error.message || `HTTP ${error.status}`;
    }
